...
def read_version_from_cmakelists(cmake_file):
    ...
    major = re.findall(
        'set\(CPACK_PACKAGE_VERSION_MAJOR.*"(.*)"', open(cmake_file).read()
    )[0]
    minor = re.findall(
        'set\(CPACK_PACKAGE_VERSION_MINOR.*"(.*)"', open(cmake_file).read()
    )[0]
    patch = re.findall(
        'set\(CPACK_PACKAGE_VERSION_PATCH.*"(.*)"', open(cmake_file).read()
    )[0]
